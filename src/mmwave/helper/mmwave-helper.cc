...
MmWaveHelper::InstallSingleUeDevice (Ptr<Node> n)
{
	...
	phy->SetPhyArchitecture(m_uePhyArch);	// Carlos modification

	Ptr<MmWavePhyMacCommon> phyMacCommon = Copy<MmWavePhyMacCommon>(m_phyMacCommon); // Carlos modification
	Ptr<MmWaveHarqPhy> harq = Create<MmWaveHarqPhy> (phyMacCommon->GetNumHarqProcess ());
...
//	ulPhy->SetHarqPhyModule (harq);
	...
	// Carlos Modification
	m_ueMmwaveNetDevices.Add(device);
//	phy->SetMmwaveEnbNetDeviceDeployedList((m_enbMmwaveNetDevices.front()));
//	phy->SetMmwaveUeNetDeviceDeployedList((m_ueMmwaveNetDevices.front()));
	// End of modification
...
//	ulPhy->SetHarqPhyModule (harq);
	...
	*/
	sched->ConfigureCommonParameters (phyMacCommon);
	...
	// Carlos Modification
	m_enbMmwaveNetDevices.Add(device);
//	phy->SetMmwaveEnbNetDeviceDeployedList((m_enbMmwaveNetDevices.front()));
//	phy->SetMmwaveUeNetDeviceDeployedList((m_ueMmwaveNetDevices.front()));
	// End of modification
...
void
MmWaveHelper::InitiateChannel (NetDeviceContainer ueDevices, NetDeviceContainer enbDevices)
{
	NS_LOG_FUNCTION(this);
...
//	}
	...
	if(m_channelModelType == "ns3::MmWaveBeamforming")
	{
//		m_beamforming->Initial(ueDevices,enbDevices);	//Carlos modification (commented line)
		m_beamforming->InitialModified(ueDevices,enbDevices);	//Carlos modification
	}
	else if(m_channelModelType == "ns3::MmWaveChannelMatrix")
	{
...
//		m_3gppChannel->Initial(ueDevices,enbDevices);
		m_3gppChannel->Initial_mod(ueDevices,enbDevices);
	}
}
...
// Carlos modification
NetDeviceContainer
MmWaveHelper::GetMmwaveEnbNetDeviceDeployedList ()
{
	return m_enbMmwaveNetDevices;
}

NetDeviceContainer
MmWaveHelper::GetMmwaveUeNetDeviceDeployedList ()
{
	return m_ueMmwaveNetDevices;
}
...
{
	NS_LOG_FUNCTION (this);
	for(NetDeviceContainer::Iterator it = m_ueMmwaveNetDevices.Begin(); it != m_ueMmwaveNetDevices.End(); ++it)
	{
		Ptr<NetDevice> node = *it;
		Ptr<MmWaveUeNetDevice> mmNode = DynamicCast<MmWaveUeNetDevice>(node);
		Ptr<MmWavePhy> p = mmNode->GetPhy();//DynamicCast<MmWavePhy>(*it);
		p->SetMmwaveEnbNetDeviceDeployedList(m_enbMmwaveNetDevices);
		p->SetMmwaveUeNetDeviceDeployedList(m_ueMmwaveNetDevices);
	}
	for(NetDeviceContainer::Iterator it = m_enbMmwaveNetDevices.Begin(); it != m_enbMmwaveNetDevices.End(); ++it)
	{
		Ptr<NetDevice> node = *it;
...
//	Ptr<MmWaveNetDevice> p = DynamicCast<MmWaveNetDevice>(m_ueMmwaveNetDevices.front());
//	phy->SetMmwaveUeNetDeviceDeployedList(p);
//	p = DynamicCast<MmWaveNetDevice>(m_enbMmwaveNetDevices.front());
//	phy->SetMmwaveEnbNetDeviceDeployedList(p);
}

void
MmWaveHelper::SetEnbPhyArchitecture (Architecture arch)
{
	m_enbPhyArch = arch;
}

void
MmWaveHelper::SetUePhyArchitecture (Architecture arch)
{
	m_uePhyArch = arch;
}
