...
void
MmWaveBeamforming::InitialModified(NetDeviceContainer ueDevices, NetDeviceContainer enbDevices)
{
	for (NetDeviceContainer::Iterator i = ueDevices.Begin(); i != ueDevices.End(); i++)
	{
		for (NetDeviceContainer::Iterator j = enbDevices.Begin(); j != enbDevices.End(); j++)
...
//			if(m_update)
	...
	Simulator::Schedule (m_longTermUpdatePeriod, &MmWaveBeamforming::InitialModified,this,ueDevices,enbDevices);
...
void
MmWaveBeamforming::SetBeamSweepingVector (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
	key_t key = std::make_pair(ueDevice,enbDevice);
	std::map< key_t, Ptr<BeamformingParams> >::iterator it = m_channelScanningMatrixMap.find(key);
	NS_ASSERT_MSG (it != m_channelScanningMatrixMap.end (), "could not find");
	Ptr<BeamformingParams> bfParams = it->second;
	...
	bfParams->m_enbW = enbBeamMng->GetBeamSweepVector();
	bfParams->m_ueW = ueBeamMng->GetBeamSweepVector();
	...
	UpdateChannelMatrixAntennaSpatialSignatures(a,b,bfParams);
	bfParams->m_beam = GetLongTermFading(bfParams);
	...
	SpectrumValue experiencedSinr = BeamMagic(dummyPsd,enbDevice,ueDevice);

	// Now add the experienced SINR to the beam management node
	ueBeamMng->AddEnbSinr(
			enbDevice,
			enbBeamMng->GetCurrentBeamId(),
			ueBeamMng->GetCurrentBeamId(),
			experiencedSinr);
...
void
MmWaveBeamforming::UpdateBfChannelMatrix(Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice, BeamPairInfoStruct bestBeams)
{
	key_t key = std::make_pair(ueDevice,enbDevice);
	std::map< key_t, Ptr<BeamformingParams> >::iterator it = m_channelMatrixMap.find(key);
	NS_ASSERT_MSG (it != m_channelMatrixMap.end (), "could not find");
	...
	bfParams->m_enbW = pMngEnb->GetBeamSweepVector(bestBeams.m_txBeamId);
	bfParams->m_ueW = pMngUe->GetBeamSweepVector(bestBeams.m_rxBeamId);
	...
	enbAntennaArray->SetBeamformingVector(bfParams->m_enbW);
...
void
MmWaveBeamforming::InitializeBeamsweeping (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
	key_t key = std::make_pair(ueDevice,enbDevice);
}
...
void
MmWaveBeamforming::SetInitialBeamformingVectors (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
	key_t key = std::make_pair(ueDevice,enbDevice);
	std::map< key_t, Ptr<BeamformingParams> >::iterator it = m_channelMatrixMap.find(key);
	NS_ASSERT_MSG (it != m_channelMatrixMap.end (), "could not find");
...
void
MmWaveBeamforming::UpdateChannelMatrixAntennaSpatialSignatures(Ptr<const MobilityModel> a,Ptr<const MobilityModel> b,Ptr<BeamformingParams> bfParams) const
{
	// Get the phi angle between eNB and UE
	Vector locationA = a->GetPosition ();
	Vector locationB = b->GetPosition ();
	Angles pathAngles (locationB, locationA);
	double angle_tx = pathAngles.phi;
...
//	}
	...
	complexVector_t a_tx, a_rx;
	for (unsigned txAntennaIndex = 0; txAntennaIndex < m_enbAntennaSize; txAntennaIndex++)
	{
		double phase = M_PI*sin(angle_tx)*txAntennaIndex;
		double power = 1/sqrt(m_enbAntennaSize);
		a_tx.push_back(exp(std::complex<double>(0,1)*phase)*power);
	}
	double angle_rx = angle_tx + M_PI;
	if(angle_rx > 2*M_PI)
	{
		angle_rx -= 2*M_PI;
	}
	for (unsigned rxAntennaIndex = 0; rxAntennaIndex < m_ueAntennaSize; rxAntennaIndex++)
	{
		double phase = M_PI*sin(angle_rx)*rxAntennaIndex;
		double power = 1/sqrt(m_ueAntennaSize);
		a_rx.push_back(exp(std::complex<double>(0,1)*phase)*power);
	}

	bfParams->m_channelMatrix.m_enbSpatialMatrix.clear();
	for(unsigned path = 0; path<m_pathNum; path++)
	{
		bfParams->m_channelMatrix.m_enbSpatialMatrix.push_back(a_tx);
		bfParams->m_channelMatrix.m_ueSpatialMatrix.push_back(a_rx);
	}
...
)
{
	...
	key_t dlkey = std::make_pair(ueNetDevice,enbNetDevice);
	key_t ulkey = std::make_pair(enbNetDevice,ueNetDevice);

	std::map< key_t, Ptr<BeamformingParams> >::iterator it;
	if (m_channelScanningMatrixMap.find(dlkey) != m_channelScanningMatrixMap.end ())
	{
		// this is downlink case
//			downlink = true;
//		enbDevice = txDevice;
//		ueDevice = rxDevice;
		it = m_channelScanningMatrixMap.find(dlkey);
	}
	else if (m_channelScanningMatrixMap.find(ulkey) != m_channelScanningMatrixMap.end ())
	{
		// this is uplink case
//			downlink = false;
//		ueDevice = txDevice;
//		enbDevice = rxDevice;
		it = m_channelScanningMatrixMap.find(ulkey);
	}
	...
	Ptr<BeamformingParams> bfParams = it->second;
	Ptr<SpectrumValue> bfPsd = GetChannelGainVector (txPsd, bfParams, relativeSpeed);
	SpectrumValue bfGain = (*bfPsd)/(*txPsd);
//	std::cout << "bfGain=" << Sum (bfGain)/nbands <<",bfPsd=" << Sum(*bfPsd)/nbands << std::endl; //",rxPsd=" << Sum(*rxPsd)/nbands << std::endl;
...
//
////	std::cout << "bfGain=" << Sum (bfGain)/nbands <<",bfPsd=" << Sum(*bfPsd)/nbands << std::endl; //",rxPsd=" << Sum(*rxPsd)/nbands << std::endl;
////	std::cout << "SNR=" << Sum(Sinr)/nbands << std::endl; //",rxPsd=" << Sum(*rxPsd)/nbands << std::endl;
////	std::cout << "SNR=" << 10 * log10(Sum(Sinr)/nbands) << "dB" << std::endl;
//
//
...
//
//	Ptr<MmWaveEnbNetDevice> EnbDev =
//				DynamicCast<MmWaveEnbNetDevice> (enbNetDevice);
//	Ptr<MmWaveUeNetDevice> UeDev =
//				DynamicCast<MmWaveUeNetDevice> (ueNetDevice);
//
//	// Trying to invoke the original method that calculates the pathloss
//	Ptr<MmWaveEnbNetDevice> pMmwaveEnbDevice= DynamicCast<MmWaveEnbNetDevice>(enbNetDevice);
//	Ptr<MmWaveUeNetDevice> pMmwaveUeDevice = DynamicCast<MmWaveUeNetDevice>(ueNetDevice);
//
//	Ptr<MmWaveEnbPhy> pMmwaveEnbPhy = pMmwaveEnbDevice->GetPhy();
//	Ptr<MmWaveUePhy> pMmwaveUePhy = pMmwaveUeDevice->GetPhy();
//
...
////	Ptr<SpectrumValue> rxPsd = pMultiSpectrumChannel->GetSpectrumPropagationLossModel()->CalcRxPowerSpectralDensity(txPsd,a,b);
//	Ptr<PropagationLossModel> pLossModel = pMultiSpectrumChannel->GetPropagationLossModel();
//	double powerDb = 0;
//	double pathLossDb = pLossModel->CalcRxPower(powerDb,a,b);
//	//
////	SpectrumValue test = (*rxPsd)/(*noisePsd);
//
////	double pathLossDb = 0;
////	// Get the antenna gains
////	Ptr<AntennaModel> txAntenna = EnbDev->GetPhy()->GetDlSpectrumPhy()->GetRxAntenna();
////	Ptr<AntennaModel> rxAntenna = UeDev->GetPhy()->GetDlSpectrumPhy()->GetRxAntenna();
////	double txAntennaGain = txAntenna->GetGainDb(txAngles);
////	double rxAntennaGain = rxAntenna->GetGainDb(rxAngles);
//////	std::cout << "tx:" << txAntennaGain <<" rx="<< rxAntennaGain << std::endl;
////	pathLossDb -= (txAntennaGain + rxAntennaGain);
////
...
//////	double shadowingDb = normalVariable->GetValue()*sigma;
////	alpha = 72.0;
////	beta = 2.92;
////	double lossDb = alpha + beta * 10 * log10(distance);// + shadowingDb;
////	pathLossDb -= lossDb;
//
//	Sinr = Sinr / std::pow (10.0, (-pathLossDb) / 10.0); //(10 * exp(-pathLossDb/10));
//
////	std::cout << "SNR=" << Sum(Sinr)/nbands << std::endl; //",rxPsd=" << Sum(*rxPsd)/nbands << std::endl;
////	std::cout << "SNR=" << 10 * log10(Sum(Sinr)/nbands) << "dB" << std::endl;
//
