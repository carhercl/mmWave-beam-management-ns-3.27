...
//	cmd.AddValue("interPacketInterval", "Inter-packet interval [us])", interPacketInterval);
	...
	cmd.AddValue("pathrange", "Number of the path range in the whole UE trip path, only for walking speed", pathrange);
  ...
  mmwaveHelper->SetSsBurstSetPeriod(ssburstset);
	...
	if (los_profile > 0)
	{
		...
		if(speed < 5 )
		{
			...
			Ptr < Building > building1;
			building1 = Create<Building> ();
			building1->SetBoundaries (Box (-78+10*pathrange, -77.0+10*pathrange,
										-49.8, -49.9,
										0.0, 40));

			Ptr < Building > building2;
			building2 = Create<Building> ();
			building2->SetBoundaries (Box (-76.5+10*pathrange,-76.0+10*pathrange,
										-49.8, -49.9,
										0.0, 40));

			Ptr < Building > building3;
			building3 = Create<Building> ();
			building3->SetBoundaries (Box (-74+10*pathrange,-73.5+10*pathrange,
										-49.8, -49.9,
										0.0, 40));

			Ptr < Building > building4;
			building4 = Create<Building> ();
			building4->SetBoundaries (Box (-72.5+10*pathrange,-71.5+10*pathrange,
										-49.8, -49.9,
										0.0, 40));
	...
	if (sWalk == "linear")
	{
	  ...
	  if(speed == 1.0 && pathrange > 0) {
		  uePositionAlloc->Add (Vector (-82.0 + 10.0*pathrange, -50.0, hUT));
		  simStopTime = 12;
	  }
	  else {
